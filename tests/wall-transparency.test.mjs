...
test('Scene3DRenderer reduz a opacidade da FACE e da TAMPA DE TOPO da parede quando a camada está ligada — sem tocar em nada mais (clique/seleção continuam do jeito que estavam)', () => {
  ...
  assert.ok(0.28 < 1);
  const faceMatBlock = rendererSource.slice(rendererSource.indexOf('var faceMat = new FaceMaterialClass'), rendererSource.indexOf('var faceMat = new FaceMaterialClass') + 1700);
...
test('wallsTransparent é calculado ANTES de ser usado (bug de ordem já corrigido nesta sessão)', () => {
  ...
  const faceMatIndex = rendererSource.indexOf('var faceMat = new FaceMaterialClass');
...
test('paredes de fechamento do telhado superior possuem face interna completa e acompanham a transparência das paredes', () => {
  ...
  assert.match(visualVolumeBlock, /var halfWallModel = \(Core\.WALL_THICK \/ 2\) \* Core\.GRID/);
  assert.match(visualVolumeBlock, /volumeX1 = Math\.min\(roof\.x1, roof\.x2\) \+ halfWallModel/);
  assert.match(visualVolumeBlock, /volumeY2 = Math\.max\(roof\.y1, roof\.y2\) - halfWallModel/);
