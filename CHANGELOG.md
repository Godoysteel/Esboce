...
- Adicionada a categoria **Envidraçamento**, com a primeira ferramenta **Fachada**: o botão cria um painel de vidro solto na cena (visível como placeholder por enquanto). O painel vai se encostar numa parede por ímã e recortar apenas a camada visível dela — a parede continua contando normalmente no quantitativo de alvenaria — com preenchimento em grid 2D de perfis pretos (verticais mais robustos, horizontais mais finos, moldura de contorno na largura do perfil vertical), vidros de encaixe exato nos dois eixos e junta de 10mm entre eles; isso ainda está em desenvolvimento e chega numa próxima atualização. Substitui por completo a fachada de vidro paramétrica anterior (botão 🪟 numa parede inteira), que nunca chegou a ser publicada. Base de dados (tipos, algoritmo de layout, persistência em schema versão 5) e o botão de criação já disponíveis, com 115 testes automatizados. Ver DEC-56 no Registro de Decisões Técnicas (revisão da DEC-55).
