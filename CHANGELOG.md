...
- Refeito: a "Nova divisória" de drywall deixa de ser uma parede de verdade (que dividia o cômodo em dois) e vira um objeto solto e independente — "Divisória livre". Clique no botão pra criar uma, já paralela à parede mais próxima; arraste o corpo livremente pra posicionar (sem grade), as alças das pontas pra ajustar o comprimento, as de topo/base pra altura e elevação, e uma alça verde nova pra girar livremente em qualquer ângulo (a única do app — todo outro objeto solto só gira em passos de 90°). As pontas têm um "ímã" só pra não ficarem dentro de uma parede. Escolha o tipo (espessura real: 73/95/115/145mm) num painel ao lado da peça selecionada. Não mexe em paredes nem em cômodos. Ver DEC-229.
- Novo: página "Celeiro americano metálico" (`/celeiro/`) com configurador 3D realista — escolha entre Galpão Fechado, Galpão Aberto e Celeiro, configure cor, tamanho, cobertura, portas, janelas e portões, veja a estimativa na hora e envie o pedido de orçamento pelo WhatsApp. Preços ainda provisórios. Ver DEC-230.
...
- Corrigido: a tabeira de um telhado duas-águas podia avançar por cima do oitão de um telhado transversal, mesmo o sombreamento por pixel (que resolve o caso acima) já rodando certo — quando os dois telhados compartilham a mesma parede, a borda de um cai EXATAMENTE em cima do limite da caixa de recorte do outro, e o teste de caixa do shader (comparação estrita `<`/`>`) excluía esse pixel do teste de altura por causa desse empate exato. Corrigido com uma margem de 1mm na caixa (imperceptível). Ver DEC-215.
- Novo: a navegação do viewport 3D agora tem três modos escolhíveis (botão "Nav", ao lado do "Orbit") — **Fácil** (novo, e agora o padrão): botão direito só pana, girar é arrastando a bússola no canto da tela, nunca precisa saber de nenhum atalho escondido; **Blender**: o esquema de sempre do Esboce (botão direito/meio gira, Shift+arraste pana); **Revit**: botão do meio pana, Shift+botão do meio gira, botão direito continua livre pro menu de contexto. A escolha fica salva no navegador. Além disso, o zoom da roda do mouse agora sempre mira o ponto embaixo do cursor (não um alvo fixo no centro) — vale nos três modos, inclusive Blender, e evita a cena "escorregar" ao dar zoom longe do centro da tela. Ver DEC-216.
- Novo: no modo Fácil, além da bússola, agora também dá pra girar a câmera segurando e arrastando o botão esquerdo do mouse em área VAZIA (sem clicar em nada) — só quando isso não faria mais nada de qualquer forma (sem ferramenta de desenho ativa, sem objeto embaixo do cursor); clicar em cima de qualquer coisa continua selecionando/editando normalmente, sem girar. Ver DEC-216.
- Removido: a ferramenta "Desenhar" (parede livre à mão, painel Paredes) — decisão de produto, não bug. "Drywall" (divisória interna) e "Desenhar ambiente livremente"/Cômodo de tamanho livre continuam do mesmo jeito; editar parede já existente (arrastar ponta/corpo) também não muda. Ver DEC-217.
- Corrigido: o orçamento de um projeto Steel Frame vinha com cimento, cal e areia de reboco somados no mesmo total — item de acabamento de parede de alvenaria que não existe nesse sistema (o fechamento de Steel Frame é placa, não reboco). Agora esse item só entra pra sistemas com composição cerâmica, igual já valia pro bloco/tijolo. Além disso, o PDF isolado de Steel Frame (menu Quantitativo → "Steel Frame") deixou de mostrar "—" em Preço/Valor total pros itens que já têm preço de mercado pesquisado — vira um orçamento completo por si só, sem precisar abrir o PDF Geral. Ver DEC-218.
- Novo: tutorial guiado interativo de 7 passos pra quem está usando o Esboce pela primeira vez — cria um cômodo e posiciona, cria outro e conecta pelas paredes, coloca porta, janela, forro de drywall, telhado e gera o orçamento. Cada passo trava o resto da tela e só libera (piscando em verde) o botão daquele passo, com uma caixa de instrução explicando o que fazer; um botão "Pular tutorial" sempre visível sai a qualquer momento. Abre sozinho na primeira visita (só em projeto vazio) e pode ser reaberto quando quiser pelo botão "Ajuda", ao lado do Catálogo. Ver DEC-219.
- Corrigido: no tutorial guiado acima, a caixa de instrução do passo "Forro de Drywall" (e de qualquer outro passo mais abaixo na lista de um painel) podia cobrir parte do painel e sua barra de rolagem, dificultando chegar no botão certo. Agora o painel rola sozinho até mostrar o botão do passo atual, e a caixa de instrução sempre aparece fora do painel, nunca em cima dele. Ver DEC-219.
- Corrigido: o oitão (parede triangular da empena) de um telhado 2 Águas ou 1 Água sem nenhum acabamento escolhido aparecia visivelmente mais escuro/acinzentado que a parede idêntica logo abaixo dele, mesmo os dois usando a mesma cor padrão — faltava no oitão o mesmo ajuste de luz que a parede já tinha pra não ficar com um tom acinzentado sob a luz da cena. Agora os dois ficam com a cor igual. Ver DEC-220.
...
- Melhorada a performance do telhado: arrastar a cumeeira (inclinação), o parapeito ou a elevação individual (ático/telhado independente) não reconstrói mais a cena inteira a cada pixel de movimento do mouse — agora mostra um fantasma leve durante o arraste (mesma técnica já usada em redimensionar borda/mover o telhado inteiro) e só grava de verdade ao soltar. O fantasma que segue o mouse antes de colocar um telhado novo também parou de disparar mais reconstruções do que a tela consegue mostrar. Ver DEC-202.
- Corrigido: a face de uma parede sem acabamento escolhido aparecia num tom cinza-azulado em vez de branca, por causa da mistura das luzes coloridas da cena (céu, chão e preenchimento) incidindo sobre o material. Agora essa face recebe um leve reforço de branco por cima da iluminação normal, o suficiente pra corrigir o tom sem ficar chapada/sem sombra — acabamentos reais (cerâmica, textura, Steel Frame) continuam exatamente como antes. Ver DEC-203.
- Corrigido: num telhado quatro-águas em L encostando num telhado vizinho QUADRADO (hip puro, sem cumeeira própria), a cumeeira central quebrava na metade — a correção que apara a cumeeira contra um vizinho mais alto comparava contra o PICO único do vizinho, mas um hip quadrado só é tão alto assim bem no meio dele, ficando bem mais baixo perto da própria borda. Agora compara a altura REAL do vizinho exatamente no ponto onde a cumeeira encosta nele. Ver DEC-204.
- Corrigido: num telhado quatro-águas em L com dois telhados de tamanhos BEM diferentes, o espigão de canto do telhado MAIOR que encosta na aresta do menor sumia sem a peça de cumeeira (deixando uma fresta na água-furtada) — a regra que esconde esse canto foi validada só pra telhados do mesmo tamanho (pico igual) e escondia incondicionalmente mesmo quando o vizinho menor não cobre o espigão de verdade. Agora só esconde quando o vizinho não é claramente mais baixo. Ver DEC-205.
- Ajustado (pedido de desenho, não bug de altura): quando o espigão de canto de um telhado atravessa um telhado vizinho menor, agora ele interrompe exatamente na cumeeira do vizinho, em vez de continuar por cima dela até o próprio pico — mesmo o telhado maior continuando mais alto depois do cruzamento. Ver DEC-206.
- Nova ferramenta nativa "Marcador" (em Mais → Ferramentas): clique no ponto inicial e no ponto final de qualquer defeito visual pra apontar exatamente onde ele está — marca com uma esfera vermelha/azul e uma linha na própria cena 3D (fica visível no print), mostra as coordenadas e a distância entre os dois pontos, e nunca apaga nem oculta nada do projeto. Ver DEC-207.
- Corrigido, depois revertido no mesmo dia (DEC-208, testado ao vivo pelo Product Owner e confirmado incorreto — ver DEC-209): uma tentativa de fazer o espigão "reaparecer" perto do próprio pico depois de sair da pegada do vizinho piorou o resultado, criando um vão visível no meio dele. Confirmado que o comportamento correto é o original: uma vez que o espigão cruza a cumeeira de um vizinho, ele fica cortado dali pra frente até o próprio pico, sem reaparecer. Ver DEC-209.
...
- Corrigido (de novo): em blocos com proporção de parede (largo/alto, profundidade fina), o painel ACM ainda ficava visualmente "dentro" da estrutura — a correção anterior (DEC-191) empurrava os cantos na diagonal a partir do centro do bloco, direção dominada pelas dimensões grandes, deixando quase nenhum deslocamento real na face pintada. Agora cada face é deslocada 4mm na própria normal (como pedido) mais um afastamento no próprio plano da face — sem reabrir a fresta nas arestas. Ver DEC-193.
- Cubo mágico "Metalão": os montantes/travessas intermediários (a cada 1200mm) agora também se repetem nas faces LATERAIS (esquerda/direita) — antes só apareciam na frente/fundo, deixando um bloco de planta quadrada (ex. um pilar/torreão revestido) sem nenhum reforço no meio das laterais. Corrigido (de novo, com dados reais de um bloco 1×1,7×1m): o painel ACM ainda deixava uma fresta bem fina na aresta entre duas faces pintadas quando o bloco não tinha proporção de parede — a correção anterior (DEC-193) empurrava o canto por um vetor diagonal de comprimento fixo, que numa face não quadrada perde alcance no eixo mais curto; agora cada face é alargada pelo valor cheio nos dois eixos do seu próprio plano, independentemente da proporção. Ver DEC-194.
