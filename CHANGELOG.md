...
- Adicionada a categoria **Envidraçamento**, com a primeira ferramenta **Fachada**: painel de vidro livre, desenhado e redimensionado na viewport (mesmo padrão de Laje), que se encosta numa parede por ímã e recorta apenas a camada visível dela — a parede continua contando normalmente no quantitativo de alvenaria. O preenchimento é um grid 2D de perfis pretos (verticais mais robustos, horizontais mais finos, moldura de contorno na largura do perfil vertical) com vidros de encaixe exato nos dois eixos (largura e altura calculadas independentemente, mirando uma largura-alvo de módulo configurável) e junta de 10mm entre vidros. Substitui por completo a fachada de vidro paramétrica anterior (botão 🪟 numa parede inteira), que nunca chegou a ser publicada. Nesta entrega: tipos, algoritmo de layout (`Glazing.ts`) e persistência (schema versão 5), com 115 testes automatizados. Comandos de criação/arraste no editor e a renderização 3D dos perfis e do vidro ficam para a próxima entrega. Ver DEC-56 no Registro de Decisões Técnicas (revisão da DEC-55).
